...
class HTTPInterface:
    ...
    def _validate_body(self, content_type):
        ...
        if 'application/json' in content_type:
            ...
            if not self.auto_format:
                try:
                    json.loads(self.body if isinstance(self.body, str) else json.dumps(self.body))
                    return self.body
                except json.JSONDecodeError:
                    raise ValueError("Invalid JSON body for Content-Type application/json")
                    
        elif 'application/xml' in content_type or 'text/xml' in content_type:
            if self.auto_format and isinstance(self.body, str):
                try:
            ...
            if not self.auto_format:
                try:
                    ET.fromstring(self.body if isinstance(self.body, str) else str(self.body))
                    return self.body
                except ET.ParseError:
                    raise ValueError("Invalid XML body for Content-Type application/xml")
                    
        elif 'application/x-www-form-urlencoded' in content_type:
            if self.auto_format and isinstance(self.body, dict):
                return self.body
            if not self.auto_format and not isinstance(self.body, dict):
    ...
    def send_get(self, endpoint, params=None):
        ...
        formatted_body = self._validate_body(content_type)
        
        return self.session.get(url, headers=self.headers, params=params, data=formatted_body)
        
    def send_post(self, endpoint):
        url = urljoin(self.base_url, endpoint.lstrip('/'))
        content_type = self.headers.get('Content-Type', '')
        formatted_body = self._validate_body(content_type)
        
        return self.session.post(url, headers=self.headers, data=formatted_body)
    ...
    def send_custom(self, method, endpoint, params=None):
        ...
        return self.session.request(method.upper(), url, headers=self.headers, params=params, data=formatted_body)
