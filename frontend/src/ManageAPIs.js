...
async function deleteApiOnServer(api_id, user_id) {
  ...
  if (!res.ok || !data.success) {
    throw new Error(data.message || 'Failed to delete API from server');
  }
  return data;
}
...
async function fetchUserApis(userId) {
  try {
...
const ManageAPIs = () => {
  ...
  useEffect(() => {
    const loadUserApis = async () => {
      if (userId) {
        setIsLoading(true);
        const serverApis = await fetchUserApis(userId);
        const normalizedApis = serverApis.map(api => ({
          ...api,
          //api_id: api.id || api.api_id,
          filename: api.name,
          uploadDate: api.created_at ? new Date(api.created_at).toISOString().split('T')[0] : 'N/A',
        }));
  ...
  useEffect(() => {
    return () => scanMonitoringService.stopAllMonitoring();
  ...
  const fetchPastScans = async (api) => {
    ...
    try {
      const listRes = await fetch(`/api/scan/list?user_id=${user_id}&api_id=${api_id}`, { method: 'GET', headers: { 'Content-Type': 'application/json' }, credentials: 'include', });
      const listBody = await listRes.json();
      if (!listBody.success || !Array.isArray(listBody.data.scans) || listBody.data.scans.length === 0) {
        showMessage('No past scans found for this API.', 'info');
        return;
      }
      const detailedScansPromises = listBody.data.scans
        .filter(scan => scan.status === 'completed')
        .map(async (scan) => {
          const detailsRes = await fetch(`/api/scan/details`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
          ...
          if (detailsBody.success && detailsBody.data.results) {
            return {
              scanId: scan.id,
              results: detailsBody.data.results.results || [],
              firstTimestamp: new Date(scan.completed_at || scan.started_at).getTime()
            };
          }
          return null;
        });
      const scans = (await Promise.all(detailedScansPromises)).filter(Boolean);
      if (scans.length === 0) {
        showMessage('No completed scans with results found for this API.', 'info');
        return;
      }
  ...
  const handleScanStart = async (scanProfile) => {
    ...
    try {
      const isImportedApi = apis.some(api => api.id === scanTargetApi.id);
      const isRegularApi = apis.some(api => api.id === scanTargetApi.id);
      
      let apiName;
      
      if (isImportedApi || isRegularApi) {
        apiName = scanTargetApi.filename || scanTargetApi.name || `API_${scanTargetApi.api_id}`;
      } else {
        throw new Error(`API not found. ID: ${scanTargetApi.id}`);
      }
      ...
      showMessage(`🔄 Enhanced scan started with ID: ${returnedScanId}. Monitoring...`, "info");

      startEnhancedScanMonitoring(returnedScanId, scanTargetApi, apiName);
  ...
  const startEnhancedScanMonitoring = (scanId, targetApi, apiName) => {
    scanMonitoringService.startMonitoring(scanId, {}, {
      ...
      onStepComplete: (step) => {},
      onComplete: (results) => {
        handleEnhancedScanComplete(scanId, results, targetApi);
      },
      onError: (error) => {
  ...
  const handleEnhancedScanComplete = (scanId, results, targetApi) => {
    const vulnCount = results?.results?.length || 0;
    setShowProgressModal(false);
    ...
    showMessage(`✅ Scan completed! Found ${vulnCount} vulnerabilities.`, 'success');
  };
  ...
  const apiStats = (() => {
    ...
    const issuesFound = apis.filter(api => (api.vulnerabilitiesFound || 0) > 0).length;
  ...
  const handleDeleteApi = useCallback(async (api) => {
    try {
      if (!api) return;
      const confirmDelete = window.confirm(`Are you sure you want to delete "${api.filename || api.name}"? This will permanently remove the API, its endpoints, and all scan history.`);
      if (confirmDelete) {
        setIsLoading(true);
        // The userId is available from the auth context in this component
        await deleteApiOnServer(api.id || api.api_id, userId);
        setApis(prev => prev.filter(a => a.id !== api.id));
        showMessage(`🗑️ API "${api.filename || api.name}" deleted successfully!`, 'success');
      }
    } catch (error) {
      console.error('Error deleting API:', error);
      showMessage(`❌ Error deleting API: ${error.message}`, 'error');
    } finally {
        setIsLoading(false);
    }
  }, [userId, showMessage]); // Make sure userId and showMessage are in the dependency array
  ...
  const handleDeleteImportedApi = useCallback((api) => {
    try {
      ...
      if (confirmDelete) {
        setApis(prev => prev.filter(imported => imported.id !== api.id));
  ...
  const handleImportAPISubmit = async (e) => {
    ...
    try {
      ...
      if (!filename || !api_id) throw new Error("Import response missing critical data");
  ...
  }, [apiToDelete, showMessage, setApis]);
                ...
                {apis.map((api, index) => (
                    <div key={api.id} className="api-card" style={{
                      ...
                      <div className="api-card-header">
                        <h4 className="api-name" style={{ color: 'white' }}>{api.filename || api.name}</h4>
                        <span className={`api-status ${api.status?.toLowerCase()}`} style={{
                      ...
                      <div className="api-meta" style={{ color: '#c7d2fe' }}>
                        <span>Uploaded: {api.uploadDate}</span>
